...
{
    public static string path = "Assets/Editor/Labels.asset";
    ...
    void OnEnable()
    {
        ...
        {
            Debug.LogError("Can't find the script path.");
        }
        else
        {
            var dire = Path.GetDirectoryName(scriptPath);
            if (string.IsNullOrEmpty(dire))
            {
                Debug.LogError("Can't find the script directory.");
            }
            else
            { 
                path = $"{dire}/Labels.asset";
            }
        }
