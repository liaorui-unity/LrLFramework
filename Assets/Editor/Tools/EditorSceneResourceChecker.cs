...
public class EditorSceneResourceChecker : IEquatable<EditorSceneResourceChecker>
{
	...
	public bool isgui;
	public EditorSceneResourceChecker()
	{

	}

    public bool Equals(EditorSceneResourceChecker other)
    {
    ...
    {
        return Equals(obj as EditorSceneResourceChecker);
    }
};
...
public class ResourceChecker : EditorWindow {
	...
	List<EditorSceneResourceChecker> ActiveTextures=new List<EditorSceneResourceChecker>();
	...
	private void RemoveDestroyedResources()
	{
		...
		ActiveTextures.ForEach(delegate(EditorSceneResourceChecker obj) {
		...
		foreach (EditorSceneResourceChecker tEditorSceneResourceChecker in ActiveTextures) TotalTextureMemory += tEditorSceneResourceChecker.memSizeKB;
	...
	void ListTextures()
	{
		...
		foreach (EditorSceneResourceChecker tDetails in ActiveTextures)
		{			
		...
		if (ActiveTextures.Count>0)
		{
			...
			if(GUILayout.Button("Select \n All",GUILayout.Width(ThumbnailWidth*2)))
			{
				...
				foreach (EditorSceneResourceChecker tDetails in ActiveTextures) AllTextures.Add(tDetails.texture);
	...
	EditorSceneResourceChecker FindEditorSceneResourceChecker(Texture tTexture)
	{
		foreach (EditorSceneResourceChecker tEditorSceneResourceChecker in ActiveTextures)
		{
			if (tEditorSceneResourceChecker.texture==tTexture) return tEditorSceneResourceChecker;
		}
		return null;
	...
	void CheckResources()
	{
		...
		ActiveTextures.Sort(delegate(EditorSceneResourceChecker details1, EditorSceneResourceChecker details2) { return details2.memSizeKB - details1.memSizeKB; });
	...
	private EditorSceneResourceChecker GetTextureDetail(Texture tTexture, Material tMaterial, MaterialDetails tMaterialDetails)
	{
		EditorSceneResourceChecker tEditorSceneResourceChecker = GetTextureDetail(tTexture);

		tEditorSceneResourceChecker.FoundInMaterials.Add(tMaterial);
		foreach (Renderer renderer in tMaterialDetails.FoundInRenderers)
		{
			if (!tEditorSceneResourceChecker.FoundInRenderers.Contains(renderer)) tEditorSceneResourceChecker.FoundInRenderers.Add(renderer);
		}
		return tEditorSceneResourceChecker;
	}

	private EditorSceneResourceChecker GetTextureDetail(Texture tTexture, Renderer renderer)
	{
		EditorSceneResourceChecker tEditorSceneResourceChecker = GetTextureDetail(tTexture);

		tEditorSceneResourceChecker.FoundInRenderers.Add(renderer);
		return tEditorSceneResourceChecker;
	}

	private EditorSceneResourceChecker GetTextureDetail(Texture tTexture, Animator animator)
	{
		EditorSceneResourceChecker tEditorSceneResourceChecker = GetTextureDetail(tTexture);

		tEditorSceneResourceChecker.FoundInAnimators.Add(animator);
		return tEditorSceneResourceChecker;
	}

	private EditorSceneResourceChecker GetTextureDetail(Texture tTexture, Graphic graphic)
	{
		EditorSceneResourceChecker tEditorSceneResourceChecker = GetTextureDetail(tTexture);

		tEditorSceneResourceChecker.FoundInGraphics.Add(graphic);
		return tEditorSceneResourceChecker;
	}

	private EditorSceneResourceChecker GetTextureDetail(Texture tTexture, MonoBehaviour script)
	{
		EditorSceneResourceChecker tEditorSceneResourceChecker = GetTextureDetail(tTexture);

		tEditorSceneResourceChecker.FoundInScripts.Add(script);
		return tEditorSceneResourceChecker;
	}

	private EditorSceneResourceChecker GetTextureDetail(Texture tTexture, Button button) 
	{
		EditorSceneResourceChecker tEditorSceneResourceChecker = GetTextureDetail(tTexture);

		if (!tEditorSceneResourceChecker.FoundInButtons.Contains(button))
		{
			tEditorSceneResourceChecker.FoundInButtons.Add(button);
		}
		return tEditorSceneResourceChecker;
	}

	private EditorSceneResourceChecker GetTextureDetail(Texture tTexture)
	{
		EditorSceneResourceChecker tEditorSceneResourceChecker = FindEditorSceneResourceChecker(tTexture);
		if (tEditorSceneResourceChecker == null)
		{
			tEditorSceneResourceChecker = new EditorSceneResourceChecker();
			tEditorSceneResourceChecker.texture = tTexture;
			tEditorSceneResourceChecker.isCubeMap = tTexture is Cubemap;
			...
			tEditorSceneResourceChecker.memSizeKB = memSize / 1024;
			tEditorSceneResourceChecker.format = tFormat;
			tEditorSceneResourceChecker.mipMapCount = tMipMapCount;
